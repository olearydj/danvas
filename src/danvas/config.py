...
from danvas.source_layouts import (
    NEW_PROJECT_SOURCE_LAYOUT,
    SOURCE_LAYOUT_NAMES,
    materialize_source_config,
)
from danvas.timezones import course_timezone_value, normalize_timezone, require_timezone
...
def command_init(args: Any) -> None:
    ...
    source_config = source_config_for_init(
        ...
        requested_layout=getattr(args, "source_layout", None),
    ...
    if getattr(args, "timezone", None) is None and existing_timezone:
        ...
        timezone = resolve_init_timezone(args, course)
...
def resolve_init_timezone(args: Any, course: Any) -> str | None:
    """Resolve init timezone from CLI, Canvas course metadata, then profile."""
    explicit = getattr(args, "timezone", None)
    if explicit:
        return require_timezone(explicit, source="--timezone")

    canvas_value = course_timezone_value(course)
    if canvas_value:
        normalized = normalize_timezone(canvas_value)
        if normalized:
            return normalized
        print(
            f"Canvas returned an unmapped course timezone: {canvas_value!s}. "
            "Danvas will not guess an IANA timezone.",
            file=sys.stderr,
        )

    profile_timezone = getattr(args, "profile_timezone", None)
    if profile_timezone:
        return require_timezone(profile_timezone, source="the selected profile")

    print(
        "Course timezone remains unconfigured. Date-only assignment metadata is unavailable "
        "until [canvas].timezone is set in .danvas/config.toml or `danvas init` is rerun "
        "with --timezone.",
        file=sys.stderr,
    )
    return None
...
def write_project_config(
    ...
    *,
    ...
    canvas_config = deepcopy((existing_config or {}).get("canvas") or {})
    if not isinstance(canvas_config, dict):
        raise SystemExit("[canvas] must be a TOML table.")
    if profile:
        canvas_config["profile"] = profile
    elif not existing_config:
        canvas_config.pop("profile", None)
    ...
    if timezone:
        canvas_config["timezone"] = timezone
    elif not existing_config:
        canvas_config.pop("timezone", None)

    assignment_groups: dict[str, int] = {}
    for group in course_snapshot.get("assignment_groups", []):
        name = str(group.get("name") or "")
        if name:
            assignment_groups[name] = int(group["id"])

    merged = deepcopy(existing_config or {})
    merged["canvas"] = canvas_config
    merged["assignment_groups"] = assignment_groups
    if source_config is not None:
        merged["sources"] = deepcopy(source_config)
    ...
    lines.extend(render_toml_document(merged))
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines), encoding="utf-8")


def source_config_for_init(
    *,
    existing_config: dict[str, Any],
    requested_layout: str | None,
    config_exists: bool,
) -> dict[str, Any]:
    if requested_layout is not None and requested_layout not in SOURCE_LAYOUT_NAMES:
        choices = ", ".join(SOURCE_LAYOUT_NAMES)
        raise SystemExit(f"--source-layout must be one of: {choices}.")
    if requested_layout is not None:
        return materialize_source_config({"layout": requested_layout})
    if config_exists:
        return materialize_source_config(existing_config.get("sources"))
    return materialize_source_config({"layout": NEW_PROJECT_SOURCE_LAYOUT})


def render_toml_document(data: dict[str, Any]) -> list[str]:
    """Render the bounded project-config TOML values that danvas supports."""
    if not isinstance(data, dict):
        raise SystemExit("danvas project configuration must be a TOML table.")
    lines: list[str] = []
    _render_toml_table(data, path=(), lines=lines, emit_header=False)
...
def toml_value(value: Any) -> str:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise SystemExit("Project configuration cannot contain non-finite numbers.")
        return repr(value)
    ...
    raise SystemExit(
        f"Unsupported project configuration value type: {type(value).__name__}"
    )
